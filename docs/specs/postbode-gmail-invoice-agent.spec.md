...
**Changelog:**
- **v1.11 — the watch scope was never enforced, and silently dropped imported mail.** F-11 and F-13 amended after a live defect: for two days the daemon polled successfully every 5 minutes and staged nothing, while three Anthropic invoices sat in the mailbox. Root cause was `users.history.list`'s `labelId` parameter, which F-12's implementation used to express F-11's INBOX scope. It does not mean that. Gmail matches `labelId` against the label set recorded *in the history record*, which is **empty** for messages inserted by the "Check mail from other accounts" POP3 fetcher (and by `messages.import`/`insert`), so every imported invoice was dropped before its id was ever returned — while `[SENT]` and `[DRAFT]` adds passed straight through, so the parameter did not even enforce the scope it claimed. Both halves verified live against the real mailbox: the same history window returned **0** `messagesAdded` records with `labelId=INBOX` and **3** without it. F-13's `in:inbox` fallback query had the same blind spot, so the designated safety net could not catch what the primary path missed — which is why the failure was silent and total rather than degraded. **The scope is now a single check on the fetched message's real labels** (`Watcher.inScope`), applied identically to both paths, and its default widens from INBOX-only to the whole mailbox minus `SENT`/`DRAFT`/`TRASH`/`SPAM`. `gmail.watch: inbox` still selects the old narrow scope, now correctly enforced. The tests could not have caught this: `fake.Server.HistoryFunc` takes only `(startHistoryId, pageToken)`, making `labelId` unrepresentable, and the two tests named for F-11 asserted only that the parameter was *sent*.
- **v1.10 — the review UI stops fighting the user.** F-46 amended again: the session token is persisted and reused across restarts, so a bookmarked `http://127.0.0.1:7391/` keeps working indefinitely instead of dying on every daemon restart. Rotation stays available by deleting the token file. Also a real visual design for the queue — dark-blue gradient, card layout, colour-coded badges, native `<details>` PDF preview — with **still zero client-side JS**, so NF-01 and ADR-002's browser-free e2e approach both hold. AC-33.
- **v1.9 — reachability of the review UI.** F-46 amended: the session token rotates on every daemon start and had to be in the URL, so bookmarks and open tabs turned into a bare `unauthorized` with no hint what to do. A first authenticated visit now sets a same-origin session cookie, making plain `http://127.0.0.1:7391/` work thereafter, and the 401 page names `postbode review`. Security properties unchanged: same secret, still loopback-only, forged cookies rejected.
- **v1.8 — first revision driven by the developer actually using the tool.** Two defects, both "works correctly, communicates nothing":
  1. **F-58 (new):** uploads rode the 5-minute poll ticker, so pressing Approve did nothing visible for up to five minutes. Uploads now have their own 15s cadence plus an immediate signal from the UI on approve.
  2. **F-45 amended:** the staging notification said *"N invoices waiting for review."* and stopped. An `osascript` notification is not clickable and cannot launch anything, so it announced work and gave no route to it — the first user saw it and had to ask where the invoices were. It now appends `Run: postbode review`, and the plural is fixed.
- **v1.7 — Phase 13 implementation findings.** `go-keyring` dropped from NF-13: `internal/keychain` shells out to `/usr/bin/security` (what go-keyring's darwin backend does anyway), so the dependency bought nothing. More importantly the secret is passed on **STDIN, never argv** — `security add-generic-password -w <value>` would put the PAT in a world-readable process argument list, readable by any local process via `ps aux` during the write. Fixed and verified live. Also added the `item_transition`-style additive columns the uploader needed (`first_failed_at`).
...
- **v1.4 — first revision driven by evidence from the live API rather than documentation.** Three findings, all from the Phase 3 spike:
  ...
  3. **`companyNumber` is the bare enterprise number** (`0XXXXXXXXX`), not the `BE`-prefixed form the schema's example implies. The `administrations` query returns it bare and `uploadFile` accepts it bare.
...
**Repo identity (supersedes PRD §13.2):** Go module path is `github.com/vhco-pro/postbode`. The PRD's `github.com/michielvh/postbode` is **superseded** and must not be used. Repo is `git init`-ed on `main`, commit identity `michielvha <<maintainer>>`, no remote configured, no push.
...
| F-11 | P0 | **Watch scope is the whole mailbox except `SENT`, `DRAFT`, `TRASH` and `SPAM`.** Not a dedicated label. Precision is delivered by the rules engine (§3.4) and the review queue (§3.5). Configurable via `gmail.watch`: `all` (default) or `inbox` (the pre-v1.11 narrow scope). **The scope is enforced in exactly one place — a check on the labels Gmail reports for the fetched message — and never via `history.list`'s `labelId` parameter, which is not the same predicate and drops label-less imported mail (see the v1.11 changelog entry).** | **Resolves PRD §11 open question 1.** Developer decision, this session. Reversible via config. **Amended v1.11:** the INBOX-only scope could not see mail that never carries the INBOX label — Gmail's POP3 fetcher with "Archive incoming messages" on imports invoices exactly this way — which is a silent miss, the one failure G-1 forbids outright. |
| F-12 | P0 | Poll every `gmail.poll_interval` (default `5m`) using incremental `history.list` from a stored `historyId`. | PRD §6.1. |
| F-13 | P0 | On first run, or on any history gap / `404 historyId not found`, fall back to `messages.list` with query `-in:sent -in:draft -in:trash -in:spam newer_than:{query_window_days}d (has:attachment OR invoice OR factuur)`, bounded by `gmail.query_window_days` (default 30). Under `gmail.watch: inbox` the scope clause is `in:inbox` instead. | PRD §6.1, §9. Covers the 14-day-offline case. **Ratified v1.3 (planner finding OQ-P6):** the original query was neither scoped nor date-bounded, so a resync would have swept the entire archived mailbox and staged years of old invoices on the first history gap. `newer_than:` is load-bearing, not cosmetic. **Amended v1.11:** the scope clause changed from `in:inbox` to the explicit exclusion list, tracking F-11. `in:inbox` gave the fallback the *same* blind spot as F-12's history path, so the safety net could not catch what the primary path missed — a resync is only a safety net if its scope is at least as wide as the thing it backstops. |
...
| F-45 | P0 | macOS notification via `osascript` when new items are staged ("Postbode: N invoices waiting for review.") and when a batch of uploads completes. | PRD §5, §6.4. **Amended v1.8:** the message must also tell the user how to open the queue (`Run: postbode review`) — an osascript notification cannot be clicked or launch anything, so the wording is the only affordance there is. Plural must agree with the count. |
| F-46 | P0 | The UI is protected by a **random per-daemon-start session token** required on every mutating request, so other local processes cannot approve uploads. The daemon writes it to `~/Library/Application Support/Postbode/session.token` with mode **`0600`**, rewriting it on every start; `postbode review` reads that file and opens the browser at the tokenized URL. The path is gitignored (F-63). | PRD §8. **Ratified v1.3 (planner finding OQ-P8):** the spec required a per-start token but never said how `postbode review` — a *separate process* — learns it. Without a defined handoff the requirement is unimplementable. A `0600` file is the minimum mechanism consistent with "other local processes can't approve uploads", since it restricts to the same UID. **Amended v1.9:** a first authenticated visit also sets a `postbode_session` cookie (HttpOnly, SameSite=Strict, loopback-only), so the UI is reachable afterwards at a plain, bookmarkable `http://127.0.0.1:7391/`. The cookie carries the same secret the URL did, so it widens nothing, and a forged cookie is still rejected. A missing/stale session returns an HTML page naming `postbode review` rather than the word "unauthorized" — the overwhelmingly likely cause is a restarted daemon, not an intruder, and anyone who can reach this port can already read the 0600 token file as the same user. **Amended v1.10 — the token is now persisted and reused across restarts** (`LoadOrCreateToken`) rather than regenerated each start, which is what finally fixes the bookmark. Regenerating invalidated every bookmark and open tab on each restart — and launchd restarts on every upgrade, reboot and crash — while protecting nothing extra, since the value lives in the same `0600` file either way and anyone able to read it after a rotation could read it before. Rotation remains available by deleting the file, the documented response to a token believed leaked (pasted into a chat, caught in a screenshot). |
...
| F-58 | P0 | **Approval must act promptly.** Uploads run on their own cadence (`upload.interval`, default **15s**), independent of `gmail.poll_interval`, and the review UI signals the daemon on approve so a drain starts immediately. p95 from pressing Approve to the upload being attempted: **< 5 seconds**. | **Added v1.8 from live use.** Uploads originally rode the 5-minute poll ticker, so pressing Approve did nothing observable for up to five minutes. The queue was correct the whole time — items sat in `approved` with no error — but a correct system that looks broken for five minutes is a defect, and the first real user reported exactly that. Draining is one indexed query that almost always returns nothing, so a short cadence is effectively free. The ticker is also the backstop for retries and for approvals arriving while a drain is already queued. |
...
| F-62 | P0 | **[v1.7: implemented WITHOUT `go-keyring`.** The dependency was never actually needed: `internal/keychain` shells out to `/usr/bin/security`, which is what go-keyring's own darwin backend does anyway — one fewer dependency for identical behaviour. **The secret is written on STDIN, never as an argv element**, because argv is world-readable on macOS and `ps aux` during the write window would expose the PAT in clear text, defeating the point of the Keychain (F-55, NF-03). Verified live against macOS 26.4.]** Secrets (Gmail OAuth refresh token, ClearFacts PAT) stored in the macOS Keychain, with an env-var fallback for dev only. | PRD §6, §8, §13.2. |
...
| NF-13 | Maintainability | Dependencies restricted to: `google.golang.org/api/gmail/v1`, `golang.org/x/oauth2`, `modernc.org/sqlite`, **`gopkg.in/yaml.v3`**, plus stdlib. **`github.com/zalando/go-keyring` was dropped in v1.7** — `internal/keychain` shells out to `/usr/bin/security` instead (see F-62), so the dependency bought nothing. Any further addition requires a note in the plan. (PRD §13.2.) **Ratified v1.3 (planner finding OQ-P2):** the YAML parser was an omission, not a discretionary extra — F-26 mandates a YAML config file, and the original list contained no way to read one. A PDF **text-extraction** dependency is deliberately still excluded: F-32's "PDF text layer where one exists" is therefore a P2 refinement, and L3 ships parsing filename + subject only (see F-32 note), so no dependency decision blocks it. F-22's password-protected-PDF check needs no library — it is a stdlib scan for `/Encrypt`. |
...
>
> Also confirmed live: **`companyNumber` is the bare enterprise number** (e.g. `0XXXXXXXXX`), not the `BE`-prefixed form the schema's `BE0123456789` example implies. `administrations` returns it bare and `uploadFile` accepts it bare; the `BE`-prefixed form was tested and is not required.
...
Normative shape is PRD §6.3, plus these P1 additions: `gmail.watch: all` (F-11; `inbox` also accepted), `vendors.known_peppol: []` (F-36), `retention_days: 30` (F-24), `upload.max_concurrent: 1`, `upload.min_interval: 2s` (F-54), `ui.port: 7391`.
...
- [ ] **AC-32:** After one authenticated visit carrying `?t=<token>`, a request to `/` with **no** token parameter succeeds via the session cookie. A request with neither token nor cookie returns 401 **and** a body naming `postbode review`. A request carrying a forged cookie returns 401. *(F-46)* — *added v1.9.*
- [ ] **AC-33:** `LoadOrCreateToken` returns the same token on repeated calls against the same path, so a daemon restart never invalidates a bookmark. The file stays `0600`; deleting it rotates the token; a trailing newline in the file is tolerated rather than silently breaking auth for the whole UI. *(F-46)* — *added v1.10.*
- [ ] **AC-31:** Approving an item via the UI results in an upload attempt within **5 seconds**, without waiting for a Gmail poll. Verified with a fake clock/short intervals: `Nudge` never blocks (it runs on the HTTP request path), repeated signals coalesce to one pending drain rather than queueing, and `upload.interval` is independent of and strictly shorter than `gmail.poll_interval`. *(F-58)* — *added v1.8.*
...
- **A-15 — `companyNumber` is the bare enterprise number** (`0XXXXXXXXX`), not the `BE`-prefixed form the schema example (`BE0123456789`) implies. Confirmed live: `administrations` returns it bare, `uploadFile` accepts it bare. *[Risk: low — verified against production]*
...
- **A-4 — Watch scope is the whole mailbox minus SENT/DRAFT/TRASH/SPAM** (developer decision, resolving PRD §11 open question 1; widened from INBOX-only in v1.11 after it silently missed POP3-imported invoices), with rules + review queue providing precision. *[Risk: medium — reversible via `gmail.watch` config]*
