...
**Version:** 1.3
**Status:** Draft — **planned**; `plans/postbode-gmail-invoice-agent.md` exists and this revision ratifies the contradictions that plan surfaced.
**Changelog:**
...
| F-15 | P0 | The `vh&co/submitted` label is resolved **by exact full name at startup**. Only an **authenticated `users.labels.list` that returns successfully without the name present** counts as "absent" → fail loudly, notify, and refuse to start the uploader (the watcher may still poll and stage). Never auto-create. An auth failure, network error or pending re-auth is **explicitly not** "absent": it defers resolution and is handled by F-16. | PRD §6.1, §13.3. Hard rule. **Ratified v1.3 (planner finding OQ-P7):** label resolution requires a live token, so as originally worded "label absent" and "cannot check because the weekly re-auth is pending" were indistinguishable — F-15's hard refusal would have fired spuriously on exactly the routine event F-16 says must never crash the daemon. The two states must be distinguished before refusing. |
| F-16 | P0 | **Re-auth is a routine, expected event, not a failure.** On refresh-token expiry or `invalid_grant`: emit a macOS notification containing a one-click re-auth URL, keep the queue and all state intact, keep the process alive, retry the auth check on every poll tick, and never silently stop polling. | PRD §9. **Verified**: an External-consent-screen app in *Testing* status issues refresh tokens that **expire after 7 days**; the name/email/profile exemption does not apply because Gmail scopes are restricted. [Source: https://developers.google.com/identity/protocols/oauth2] So this fires potentially weekly. |
| F-17 | P0 | `postbode status` reports Gmail token age, computed expiry (issue time + 7 days in Testing mode), and a `re-auth needed` flag. | Derived from F-16. |
...
| F-56 | P0 | **Provenance stamping.** Every upload sets `tags: ["postbode"]` and `comment: "postbode <item-id> · <gmail-message-id> · <sha256-prefix>"`. Both are readable back via `document(id:)` → `file { tags comment }`. | The schema exposes `comment` and `tags` as writable upload arguments — **newly discovered in v1.2**. This makes every Postbode-uploaded document self-identifying *inside the portal*, so "did Postbode send this, or did it arrive via Peppol/Auto-forward?" is answerable by looking at the document itself. Cheap, and it closes the channel-attribution gap L4 exists to paper over. [Source: https://assets-prod.cdn.clearfacts.be/doc/mutation.doc.html] |
| F-57 | P2 | **Aggregate reconciliation, conditional on the F-08 spike.** If a working `type` argument is found, `postbode status --reconcile [month]` calls `companyStatistics(invoicetype: PURCHASE, startPeriod, endPeriod)` and prints portal count vs. Postbode-uploaded count for the period, flagging any divergence. Requires the `statistics` token scope. If F-08 finds no working `type`, this is dropped. | The **only** portal-side visibility that exists. It cannot identify *which* document is missing or doubled — only that the totals disagree, which is still the difference between "I think it's fine" and "the numbers match". Explicitly conditional; do not implement speculatively. See OQ-8. |
...
4. **`comment` and `tags` are writable on upload** — see F-56, which uses them to stamp provenance.
...
- [ ] **AC-30 — conditional, only if OQ-8 resolves positively.** `postbode status --reconcile <month>` prints the portal's PURCHASE count and Postbode's uploaded count for that month and exits non-zero when they diverge. Verified against a fake `companyStatistics` response. If the F-08 probe finds no working `type` argument, F-57 and this criterion are both struck. *(F-57)* — *added v1.3, planner finding OQ-P10: F-57 previously had no criterion, so a positive probe would have left the phase with nothing to verify against.*
...
|---|---|---|---|---|
| **OQ-1** | Does an app in Google OAuth **"In production" publishing status but not yet verified** escape the 7-day refresh-token expiry? Google's own documentation does **not** state this; third-party sources claim Production yields indefinite refresh tokens but this **could not be confirmed from an authoritative source**. Marked **`[unverified — verify before relying]`**. Design impact: none on correctness — F-16 makes weekly re-auth a routine, non-crashing event either way — but it decides whether re-auth is a weekly chore or a one-off. | Michiel | Before the 2-week P1 live run | No |
...
| **OQ-8** | **NEW.** Which `type` argument string does `companyStatistics(type: String!, …)` accept? It is the one parameter the published schema does not enumerate, and it gates F-57 aggregate reconciliation. Settled empirically by the F-08 spike probe (read-only). If nothing works, F-57 is dropped and no requirement changes. | Michiel | At P0 spike | No |
...
- **A-13 — Provenance is stamped into the portal via `comment` + `tags` (F-56).** Discovered in the schema, absent from the PRD. Postbode-uploaded documents become self-identifying inside ClearFacts, which is the closest thing available to channel attribution and materially reduces how often L4 teaching is needed. *[Risk: low]*
- **A-14 — Aggregate reconciliation (F-57) is conditional and P2.** `companyStatistics` is the only portal-side signal that exists; it can prove totals agree but never identify *which* document diverged. Gated on the OQ-8 spike result and requires the `statistics` token scope. Not a substitute for L1–L4. *[Risk: low — dropped cleanly if the probe fails]*
