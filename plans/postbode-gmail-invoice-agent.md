...
- [x] Watch scope is the whole mailbox minus `SENT`/`DRAFT`/`TRASH`/`SPAM`, configurable via `gmail.watch: all|inbox` (F-11) — `watchscope.go`'s `inScope`, applied in `poll.go`'s `processMessage`, tested in `scope_test.go`. **Amended v1.11:** originally `history.go`'s `effectiveLabelID`, feeding `history.list`'s `labelId`. That parameter does not express this predicate — it matches the label set in the *history record*, empty for POP3-imported mail, so it dropped real invoices while still admitting `[SENT]`/`[DRAFT]`. Scope is now checked once, on the fetched message's labels, for both the F-12 and F-13 paths.
...
- [x] Fallback on first run or any history gap / `404 historyId not found`: `messages.list` with `has:attachment OR (invoice OR factuur)` bounded by `gmail.query_window_days` (default 30). **Add a scope clause and `newer_than:{N}d` to the query** — the spec's literal query is neither scoped nor date-bounded (see OQ-P6) (F-13) — `fallback.go` (`FallbackQuery`, `fallbackList`), proven by `fallback_query_test.go`. **Amended v1.11:** the scope clause is `-in:sent -in:draft -in:trash -in:spam` (or `in:inbox` under `gmail.watch: inbox`), tracking F-11. The original `in:inbox` gave the fallback the same blind spot as the history path, so it could not backstop it.
